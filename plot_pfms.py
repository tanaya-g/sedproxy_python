...
def plot_pfms(
    ...
    # Handle metadata (e.g., from sedproxy.pfm object)
    if isinstance(df, dict) and "everything" in df:
        df = df["everything"]
    ...
    if plot_stages == "default":
        cal_type = pfm_df.attrs.get("calibration_pars", {}).get("calibration_type", "identity")
    ...
    if color_palette == "default":
        color_palette = skey["plotting.colour"].to_dict()
    if alpha_palette == "default":
        alpha_palette = skey["plotting.alpha"].to_dict()
    ...
    g = sns.relplot(
        ...
        kind="line",
        hue="stage", style="replicate", col="scale" if "scale" in df.columns else None,
